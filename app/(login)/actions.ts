...
export const signIn = validatedAction(signInSchema, async (data, formData) => {
  ...
  await Promise.all([
    setSession(foundUser as unknown as NewUser),
...
export const signUp = validatedAction(signUpSchema, async (data, formData) => {
  ...
  if (existingUser.length > 0) {
    return { error: 'Failed to create user. Please try again.', email, password } as ActionState;
  }
  ...
  const newUser: NewUser = {
    email,
    passwordHash,
    role: 'owner', // default; may be overridden by invitation
  };
  ...
  if (inviteId) {
    ...
    if (invitation) {
      ...
      return { error: 'Invalid or expired invitation.', email, password } as ActionState;
    }
  } else {
    const newTeam: NewTeam = { name: `${email}'s Team` };
    [createdTeam] = await db.insert(teams).values(newTeam).returning();
    if (!createdTeam) {
      return { error: 'Failed to create team. Please try again.', email, password } as ActionState;
    }
  ...
  await Promise.all([
    ...
    setSession(createdUser as unknown as NewUser),
  ]);
...
export const inviteTeamMember = validatedActionWithUser(
  ...
  async (data, _, user) => {
    ...
    // TODO: send invitation email including ?inviteId={id} to sign-up URL
